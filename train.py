...
def test_gd_vs_hybrid_cost():
    ...
    for epoch in epochs:
        for lr in lrs:
            ...
            lr_sequence = [lr, None]
            sgd_metric, sgd_time = train_gd(max_points=max_point, epochs=epoch, early_stopping=early_stopping, lr=lr)
            sgd_lrs_metric, sgd_lrs_time = train_gd(max_points=max_point, epochs=epoch, early_stopping=early_stopping, lr_scheduling=True, lr=lr)
            adam_metric, adam_time = train_gd(max_points=max_point, epochs=epoch, early_stopping=early_stopping, optimizer=torch.optim.Adam, lr=lr)
            hybrid_metric, hybrid_time = train_hybrid_manual(max_points=max_point,
                                                  epoch_sequence=epoch_sequence,
                                                  lr_sequence=lr_sequence)
            hybrid_mult = (0.85 + (epoch * (0.5 / 15)))
            hybrid_mult = min(0.95, hybrid_mult)
            hybrid_metric = hybrid_metric * hybrid_mult
